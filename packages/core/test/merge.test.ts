...
const store = (rows: Row[]): Store =>
  ({ v: STORE_VERSION, txns: rows.map(txn), accounts: [ACCT], categories: [], lines: [], budgets: [] });
...
describe('prune', () => {
  ...
  it('keeps a fresh tombstone, because the delete still has to travel', () => {
    const s: Store = { v: STORE_VERSION, txns: [dead], accounts: [], categories: [], lines: [], budgets: [] };
  ...
  it('never drops a live record, however old', () => {
    const s: Store = {
      ...
      accounts: [], categories: [], lines: [], budgets: [],
    };
